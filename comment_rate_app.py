...
def create_book_comment():
    ...
    # Check if all required parameters are present
    if 'book_id' not in data or 'comment_id' not in data or 'user_id' not in data or 'comment_text' not in data:
        error = [
            {
                '400 error': 'missing parameters; usage is as follows:',
                'book_id': 'INT',
    ...
    try:
        # Prepare the query to insert a new rating
        query = """
            INSERT INTO BookComments (BookID, CommentID, UserID, Comments, DateStamp)
            VALUES (%s, %s, %s, %s, %s)
        """
        # Get the current timestamp
        date_stamp = datetime.now()
        ...
        # Execute the query
        cursor.execute(query, (book_id, comment_id, user_id, comment_text, date_stamp))
